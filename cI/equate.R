...
pairs<-list( #the first two will be used to define randomly equivalent (re) groups 
...
pairs$diff.items <- list(rowSums(resp2[,1:15]),rowSums(resp2[,16:50])) #again we are grouping by items, but in a very specific way. the first list element is all hard items, the second element all easy items
...
#############################################################################################################################################
##linear equating
linear_equate<-function(nm,L) { ##recal that this is just equating scores equally distant (in SD units) from the mean
    L[[nm]][[1]]->s1
    L[[nm]][[2]]->s2
    mod<-function(x,y) {
        mean(x)->mx
...
##rows are the different ways of splitting the data from above. which of these is sensible?
